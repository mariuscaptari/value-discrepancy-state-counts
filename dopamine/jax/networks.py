...
class ClassicControlRainbowNetwork(nn.Module):
  ...
  noisy: bool = True
  dueling: bool = True
  distributional: bool = True #TODO maybe add option for non distributional
  ...
  def __call__(self, x, support, eval_mode=False, key=None):
    ...
    if self.dueling:
      ...
      value = jnp.sum(support * value, axis=1)
      q_values = jnp.sum(support * probabilities, axis=1)
    else:
    ...
    return atari_lib.RainbowNetworkType(q_values, logits, probabilities, value)
