...
class SparseCartPole(gym.Env):
    ...
    def step(self, action):
        ...
        # Get cart location (x) and pole angle (θ) from the observation
        cart_location = observation[0]
        pole_angle = observation[2]

        # Define the sparse conditions: |x| < 0.25 and cos(θ) > 0.995 https://arxiv.org/pdf/2006.01782.pdf
        if abs(cart_location) < 0.25 and math.cos(pole_angle) > 0.995:
            reward = 1
        else:
            reward = 0
...
class SparseMountainCar(gym.Env):
    ...
    def step(self, action):
        ...
        # In the MountainCar environment, the agent's position is the first item in the observation
        # The goal position is typically 0.5
        position = observation[0]

        # Modify reward to be sparse
        # Only give a reward of 1.0 if the car reaches the goal position
        if position >= 0.5:
            reward = 1.0
        else:
            reward = 0.0
...
def create_gym_environment(environment_name=None, version='v0'):
  ...
  if environment_name =='CartPole_sparse':
      env = SparseCartPole()
  elif environment_name == 'MountainCar_sparse':
