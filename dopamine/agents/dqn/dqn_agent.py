...
class DQNAgent(object):
  ...
  def _build_networks(self):
    ...
    # Softmax output layer with temperature for Boltzmann exploration
    self.Temp = tf.compat.v1.placeholder(shape=None,dtype=tf.float32)
    self.q_dist = tf.nn.softmax(self._net_outputs.q_values/self.Temp)
  ...
  def _select_action(self):
    ...
    if self.eval_mode:
      ...
      epsilon = self.epsilon_fn(
          ...
          self.epsilon_train)
    if self.exploration_method == 'e-greedy':
      ...
      Q_probs = self._sess.run(self.q_dist,feed_dict={self.state_ph: self.state,self.Temp:epsilon})
      action_value = np.random.choice(Q_probs[0],p=Q_probs[0])
      return np.argmax(Q_probs[0] == action_value)
    else:
      raise Exception("Invalid exploration mode given.")
